...
def create_schedule(subjects, teachers):
    ...
    while uncovered_subjects:
        # Пошук викладача, що покриває найбільше з невкритих предметів
        best_teacher = None
        max_coverage = 0
        ...
        for teacher in teachers:
            ...
            if coverage > max_coverage or (
                coverage == max_coverage and best_teacher and teacher.age < best_teacher.age
            ):
                max_coverage = coverage
                best_teacher = teacher

        if max_coverage == 0:
            return None  # Немає викладача, який може викладати решту предметів

        # Призначення викладача та оновлення невкритих предметів
